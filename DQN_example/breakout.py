...
def check_img_diff():
    ...
    #env.ale.lives
    print(env.ale.lives)
    print(type(env.ale.lives))
    print(env.unwrapped.ale)
#    env.unwrapped.ale.lives = 3

    for i in range(4000):
        env.render()
        obs, rew, done, info = env.step(random.randint(0, 3))
        #print(obs.shape)
        print(rew, done, info, i)
        time.sleep(0.1)
...
def main():
    ...
    W5 = tf.Variable(tf.random_normal([20, 4], stddev=0.1))
    ...
    #loss = L5[0][0] - 1
    optimizer = tf.train.AdamOptimizer(learning_rate=0.00025, epsilon=0.0001).minimize(loss)
